...
var intent_status_row: HBoxContainer
...
func _build_ui() -> void:
	...
	player_status_row = _make_status_row()
	status_row.add_child(player_status_row)

	enemy_status_row = _make_status_row()
	status_row.add_child(enemy_status_row)

	intent_status_row = _make_status_row()
	status_row.add_child(intent_status_row)
...
func _refresh_header_width() -> void:
	...
	for row in [player_status_row, enemy_status_row, intent_status_row]:
		if row != null:
			row.custom_minimum_size.x = status_width


func _make_status_row() -> HBoxContainer:
...
func _render_status_rows() -> void:
	_clear_status_rows()
	if screen_mode == SCREEN_MENU:
		return

	_render_status_row(player_status_row, [
		"PG Lv %d |" % run_state.player_level,
		"Classe: %s |" % _get_class_display_text(run_state.active_class),
		"Vita %d/%d |" % [run_state.health, run_state.max_health],
		"Stamina %d/%d |" % [run_state.stamina, run_state.max_stamina],
		"Anime %d |" % run_state.get_material("anime"),
		"Sangue %d |" % run_state.get_material("sangue"),
		"Paura %d" % run_state.fear,
	])

	if screen_mode == SCREEN_SAFE:
		_render_status_row(enemy_status_row, [
			"Nodo: %s (%s)" % [run_state.get_current_node_name(), run_state.get_current_node_type()],
			"Posizione: %d,%d" % [run_state.map_position.x, run_state.map_position.y],
		])
	elif screen_mode == SCREEN_REWARD:
		_render_status_row(enemy_status_row, [
			"Nemico sconfitto: %s" % String(combat_state.enemy.get("name", "Nemico")),
			"Puoi scegliere fino a 1 carta.",
		])
	else:
		_render_status_row(enemy_status_row, [
			"%s |" % String(combat_state.enemy.get("name", "Nemico")),
			"Vita %d/%d |" % [combat_state.enemy.get("health", 0), combat_state.enemy.get("max_health", 0)],
			"Veleno %d |" % combat_state.enemy.get("poison", 0),
			"Bruciatura %d |" % combat_state.enemy.get("burn", 0),
			"Sangue perso %d |" % combat_state.enemy.get("bleed", 0),
			"Marchio %s" % ("si" if bool(combat_state.enemy.get("marked", false)) else "no"),
		])

	var intent_parts: Array = []
	for line in intent_label.text.split("\n"):
		var clean_line: String = String(line).strip_edges()
		if not clean_line.is_empty():
			intent_parts.append(clean_line)
	_render_status_row(intent_status_row, intent_parts)


func _render_status_row(row: HBoxContainer, parts: Array) -> void:
	if row == null:
		return
	for part in parts:
		var label = Label.new()
		label.size_flags_horizontal = Control.SIZE_EXPAND_FILL
		label.text = String(part)
...
func _get_stack_card_offset(count: int, _scale: float) -> Vector2:
	if count <= 1:
		return Vector2.ZERO
	var crowding = max(0.45, 1.0 - float(max(0, count - 8)) * 0.045)
	return STACK_CARD_OFFSET * crowding
...
func _add_card_art_overlay(parent: Control, card_id: String, color: Color, _font_size: int, action_text: String = "", cost_override: int = -1) -> void:
	var card: Dictionary = GameDatabase.get_card(card_id)
	if card.is_empty():
		return
	...
	var title_size: int = clampi(roundi(14.0 * overlay_scale), 11, 15)
	var body_size: int = clampi(roundi(12.0 * overlay_scale), 7, 12)
	var number_size: int = clampi(roundi(28.0 * overlay_scale), 18, 26)

	_add_card_art_text(parent, title_text.to_upper(), Rect2(0.105, 0.020, 0.790, 0.095), Color(0.16, 0.095, 0.055), title_size, HORIZONTAL_ALIGNMENT_CENTER, VERTICAL_ALIGNMENT_CENTER, "title")
	_add_card_art_text(parent, effect_text, Rect2(0.155, 0.705, 0.690, 0.175), Color(0.105, 0.070, 0.045), body_size, HORIZONTAL_ALIGNMENT_CENTER, VERTICAL_ALIGNMENT_CENTER, "body")
	_add_card_art_text(parent, damage_text, Rect2(0.020, 0.820, 0.205, 0.165), Color(1.0, 0.88, 0.58), number_size, HORIZONTAL_ALIGNMENT_CENTER, VERTICAL_ALIGNMENT_CENTER, "number")
	_add_card_art_text(parent, "%d" % cost, Rect2(0.775, 0.820, 0.205, 0.165), Color(0.76, 0.90, 1.0), number_size, HORIZONTAL_ALIGNMENT_CENTER, VERTICAL_ALIGNMENT_CENTER, "number")


func _get_card_overlay_scale(parent: Control) -> float:
	var card_size: Vector2 = parent.size
	if card_size.y <= 1.0:
		card_size = parent.custom_minimum_size
	if card_size.y <= 1.0:
		card_size = Vector2(CARD_WIDTH, CARD_HEIGHT)
	return clamp(card_size.y / float(CARD_HEIGHT), 0.55, 1.15)
...
func _add_card_art_text(parent: Control, text: String, anchor_rect: Rect2, color: Color, font_size: int, horizontal: HorizontalAlignment, vertical: VerticalAlignment, font_role: String = "body") -> void:
	...
	label.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART if font_role == "body" else TextServer.AUTOWRAP_OFF
	label.clip_text = font_role == "body"
...
func _get_card_overlay_font(font_role: String) -> Font:
	...
	if font_role == "number":
		font.font_names = PackedStringArray(["Constantia", "Cambria", "Georgia", "Palatino Linotype", "Times New Roman", "serif"])
	elif font_role == "title":
		font.font_names = PackedStringArray(["Constantia", "Cinzel", "Georgia", "Palatino Linotype", "Times New Roman", "serif"])
	else:
